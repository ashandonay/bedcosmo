...
def transform_input_standard_normal_log_prob(experiment, samples: torch.Tensor) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        log_prob = -0.5 * (y.pow(2).sum(dim=-1) + y.shape[-1] * math.log(2.0 * math.pi))
...
def nmc_eig(
    ...
    if M_prime is not None:
        ...
        conditional_lp = sum(retrace.nodes[l]["log_prob"] for l in observation_labels).logsumexp(
            0
        ) - math.log(M_prime)
    else:
        ...
        conditional_lp = sum(
            trace.nodes[l]["log_prob"] for l in observation_labels
        )  # shape (N, num_designs)
    ...
    retrace = poutine.trace(conditional_model).get_trace(
        reexpanded_design
    )  # sample theta_n,m M times conditioned on y_n
    ...
    if not contrastive:
        ...
        marginal_lp = sum(retrace.nodes[l]["log_prob"] for l in observation_labels).logsumexp(
            0
        ) - math.log(M)
        ...
        marginal_log_probs = torch.cat(
            [
                lexpand(conditional_lp, 1),
                sum(retrace.nodes[l]["log_prob"] for l in observation_labels),
            ]
        )  # shape (M+1, N, num_designs)
        ...
        return (
            _safe_mean_terms(conditional_lp - marginal_lp_lower)[1],
            _safe_mean_terms(conditional_lp - marginal_lp_upper)[1],
        )
...
def _vnmc_eig_loss(model, guide, observation_labels, target_labels, contrastive=False):
    ...
    def loss_fn(design, num_particles, evaluation=False, **kwargs):
        ...
        if contrastive:
            ...
            theta_y_dict = {
                l: torch.cat([lexpand(trace.nodes[l]["value"], 1), theta_y_dict[l]], dim=0)
                for l in target_labels
            }
            y_dict = {
                l: torch.cat([lexpand(trace.nodes[l]["value"], 1), y_dict[l]], dim=0)
        ...
        terms += sum(
            model_trace.nodes[l]["log_prob"] for l in observation_labels
        )  # p(y_n | theta_nm, d)
...
def posterior_loss(
    ...
    if nflow:
        ...
        flattened_samples = torch.flatten(
            evaluate_samples, start_dim=0, end_dim=len(batch_shape) - 1
        )
        flattened_condition = torch.flatten(
            condition_input, start_dim=0, end_dim=len(batch_shape) - 1
        )
        ...
        trace = poutine.trace(experiment.pyro_model).get_trace(
            expanded_design
        )  # trace: graph data structure denoting relationships amongst different pyro primitives
        y_dict = {
            l: trace.nodes[l]["value"] for l in experiment.observation_labels
        }  # trace.nodes contains a collection (OrderedDict) of site names and metadata
        theta_dict = {l: trace.nodes[l]["value"] for l in experiment.cosmo_params}

        # Run through q(theta | y, d)
        conditional_guide = pyro.condition(
            guide, data=theta_dict
        )  # set the sample statements in the guide to the values in theta_dict
        ...
        terms = -sum(
            cond_trace.nodes[l]["log_prob"] for l in experiment.cosmo_params
        )  # forward pass through network and evaluate loss func
        agg_loss, loss = _safe_mean_terms(terms)
    ...
    if evaluation:
        if analytic_prior:
            ...
            prior_entropy = sum(
                trace.nodes[l]["fn"].entropy().mean(dim=0) for l in experiment.cosmo_params
            )  # shape: [num_designs]
            ...
            # First dimension is number of MC samples for sum - reason for zero index
            prior_entropy = -1 * sum(
                trace.nodes[l]["log_prob"].mean(dim=0) for l in experiment.cosmo_params
            )
            loss = prior_entropy - loss
    return agg_loss, loss
    ...
    **kwargs,
    ...
    if return_history:
        ...
        return (
            ...
            history,
        )  # calculate prior_entropy - ape (if eig=True)
...
def _posterior_loss(model, guide, observation_labels, target_labels, analytic_entropy=False):
    ...
    def loss_fn(design, num_particles, evaluation=False, **kwargs):
        ...
        # num_particles = num_samples
        expanded_design = lexpand(
            design, num_particles
        )  # expand num_particles copies to left dimension
        ...
        y_dict = {
            l: trace.nodes[l]["value"] for l in observation_labels
        }  # trace.nodes contains a collection (OrderedDict) of site names and metadata
        ...
        if evaluation and analytic_entropy:
            ...
            terms = -sum(
                cond_trace.nodes[l]["log_prob"] for l in target_labels
            )  # forward pass through network and evaluate loss func
            agg_loss, loss = _safe_mean_terms(terms)
...
class LikelihoodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.evaluation:
            ...
            for key in prior_log_probs_list[0].keys():
                prior_log_probs[key] = torch.cat([lp[key] for lp in prior_log_probs_list], dim=0)
    ...
    def _compute_prior_log_probs(self, samples, trace):
        ...
        if sed_prior is not None:
            use_flow = sed_prior.uses_flow_prior()
            if getattr(self.experiment, "transform_input", False):
                if use_flow:
                    # Gaussianized-flow-direct: score the prior at y = T(theta) with
                    # the gaussianized flow's OWN density -- no change-of-variables.
                    # The gaussianized flow natively lives in the NF coordinates, so
                    # log p_gflow(y) is already H(y)'s integrand. We deliberately do
                    # NOT use the native-flow + analytic-Jacobian "bridge": the analytic
                    # joint Jacobian under-counts the peaked ILR marginals by ~6 nats
                    # (its density is the coarse CDF-grid slope), biasing H(y) low.
                    # transform_fn = params_to_unconstrained is the SAME forward T the
                    # guide uses (nf_loss), keeping H_prior and H_post in one y-frame.
                    if not sed_prior.has_gaussianized_flow():
                        raise RuntimeError(
                            "transform_input=True with prior_source=flow requires a "
                            "gaussianized prior flow (sed_prior_flow_gaussianized.pt). "
                            "Train it with `python -m "
                            "bedcosmo.num_visits.empirical.prior_flow --space both`."
                        )
                    log_joint = sed_prior.log_prob_gaussianized_from_native(
                        samples,
                        self.experiment.params_to_unconstrained,
                        param_names=self.experiment.cosmo_params,
                    )
                    return {"joint": log_joint}
                # prior_source=kde + transform_input: gaussianizer already maps to
                # ~N(0,I), so use the standard-normal shortcut (no separate y-KDE).
    ...
    def _process_particles(self, n_particles):
        ...
        samples = torch.cat(
            [theta_dict[k].unsqueeze(dim=-1) for k in self.experiment.cosmo_params],
            dim=-1,
        )
