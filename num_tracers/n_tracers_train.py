...
def single_run(
    ...
    **kwargs,
    ...
    bed_cosmo = bed_cosmology(priors, true_n_ratios, observation_labels, z_eff, sigma_D_H, sigma_D_M, device=train_args["device"])
    ...
    if train_args["include_D_M"]:
        ...
        for i in range(len(train_args["tracers"])):
            D_H_mean = bed_cosmo.D_H_func(z_eff[i], **parameters)
        ...
        for i in range(len(train_args["tracers"])):
            ...
            D_M_mean = bed_cosmo.D_M_func(z, **parameters)
    ...
    if kwargs["brute_force"]:
        ...
        designer = ExperimentDesigner(grid_params, grid_features, grid_designs, bed_cosmo.unnorm_lfunc, mem=190000)
