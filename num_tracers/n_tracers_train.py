        ...
        for context, samples in dataloader:
            ...
            if step % 1000 == 0:
                ...
                if run_args["log_nominal_area"]:
                    nominal_samples = experiment.sample_params(posterior_flow, nominal_context, num_samples=5000).cpu().numpy()
                    ...
                    local_nominal_areas_tensor = torch.tensor([local_nominal_areas[key] for key in sorted_keys], device=current_pytorch_device)
                    ...
                    if global_rank == 0:
                        for i, pair_key in enumerate(sorted_keys):
                            ...
                            if global_value < best_nominal_area:
                                # Save checkpoint if the mean area is the best so far
                                best_nominal_area = global_value
                                mlflow.log_metric("best_avg_nominal_area", best_nominal_area, step=step)
                                mlflow.log_metric(f"best_nominal_area_{pair_name}", best_nominal_area, step=step)
